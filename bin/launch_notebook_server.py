    ...
    for r in reservations:
        for n in r.instances:
            ...
            if not n.state == "terminated":
                ...
                # Create a list of volumes that are set to delete on instance termination
                delete_on_termination_volumes = []
                t_bdm = n.block_device_mapping
                for key, value in t_bdm.iteritems():
                    logging.info("(TI) Block Device %s (%s) Delete on Terminate:  %s" % (key, value.volume_id,
                                                                                         value.delete_on_termination))
                    print "Block Device %s (%s) Delete on Terminate:  %s" % (key, value.volume_id,
                                                                             value.delete_on_termination)
                    if value.delete_on_termination:
                        logging.info("(TI) Adding %s to delete_on_termination_volumes list" % value.volume_id)
                        delete_on_termination_volumes.append(value.volume_id)
                ...
                for w in t_volumes:
                    if not any(w.id in s for s in delete_on_termination_volumes):
                        logging.info("(TI) Volume id: %s Attach State: %s" % (w.id, w.attachment_state()))

                        while w.attachment_state() == 'attached':
                            logging.info("(TI) Waiting for volume to detach. Volume %s is still attached to "
                                         "instance %s" % (w.id, n.id))
                        ...
                        logging.info("(TI) Skipping: %s In delete_on_termination_volumes list" % w.id)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--instance_type', default='t2.micro',
                        help='Type of instance to launch, Common choices are t2.micro, c4.medium, m3.xlarge for more' +
